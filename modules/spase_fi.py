...
with open("modules/spase_fi_abi.json", "r") as file:
    spase_fi_abi = json.load(file)

def spase_fi_swap(wal:standart.Wal,token1:str,token2:str,amount:int): 
    w3 = Web3(Web3.HTTPProvider('https://mainnet.era.zksync.io'))
    contractSwap = Web3.to_checksum_address('0xbE7D1FD1f6748bbDefC4fbaCafBb11C6Fc506d1d')
    contract = w3.eth.contract(address=contractSwap, abi=spase_fi_abi)
